use super::{
    gamelog::GameLog, rex_assets::RexAssets, Equipped, Hidden, HungerClock, HungerState,
    InBackpack, Map, Name, Player, Pools, Position, RunState, State, Viewshed,
};
...
fn draw_tooltips(ecs: &World, ctx: &mut Rltk) {
    let (min_x, _max_x, min_y, _max_y) = super::camera::get_screen_bounds(ecs, ctx);
    let map = ecs.fetch::<Map>();
...
pub fn ranged_target(
    ...
    let (min_x, max_x, min_y, max_y) = super::camera::get_screen_bounds(&gs.ecs, ctx);
    let player_entity = gs.ecs.fetch::<Entity>();
